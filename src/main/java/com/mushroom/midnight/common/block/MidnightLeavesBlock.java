...
public class MidnightLeavesBlock extends BlockLeaves {
    private final Supplier<Block> saplingSupplier;

    public MidnightLeavesBlock(Supplier<Block> saplingSupplier) {
        super();
    ...
    public boolean shouldSideBeRendered(BlockState state, IBlockAccess access, BlockPos pos, Direction side) {
        ...
        return !access.getBlockState(neighborPos).doesSideBlockRendering(access, neighborPos, Dist.getOpposite());
    }
    ...
    public int getMetaFromState(BlockState state) {
        return (state.get(DECAYABLE) ? 1 : 0) | (state.get(CHECK_DECAY) ? 1 : 0) << 1;
    }
