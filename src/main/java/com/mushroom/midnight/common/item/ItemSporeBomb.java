...
public class ItemSporeBomb extends Item {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int slotId, boolean isSelected) {
        if (checkExplode(world, stack)) {
            explode(BombType.values()[stack.getMetadata()], world, entity.posX, entity.posY, entity.posZ);
            stack.shrink(1);
        }
    }
    ...
    public boolean onEntityItemUpdate(EntityItem entityItem) {
        ...
        if (valid) {
            explode(BombType.values()[stack.getMetadata()], entityItem.world, entityItem.posX, entityItem.posY, entityItem.posZ);
            entityItem.setDead();
        }
        return valid;
    }
    ...
    private static boolean checkExplode(World world, ItemStack stack) {
